...
@app.route('/')
def home():
    return "😈 Bot is Live & Naughty! 24/7"
...
async def check_force_join(user_id, context):
    ...
    for ch_id, title, link in channels:
        try:
            ...
            # If bot is not admin or channel invalid, treat as not joined or ignore
            # Safest is to prompt join if we can't verify
            pass 
    return not_joined
...
async def verify_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not_joined:
        ...
        # Trigger welcome logic manually
        text, media_id = get_welcome_data()
        caption = f"😈 *Hello {user.mention_html()}* 😈\n\n{text}"
        kb = [
            [InlineKeyboardButton("📂 Files", callback_data="files"), InlineKeyboardButton("🔗 Links", callback_data="links")],
...
async def confirm_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Post Logic with Hidden Link Check Button
    # Note: We send a "Check Link" button. When clicked, it checks Force Join logic.
    # To simplify for "Multi Post" broadcasting, we often just send the link OR use a bot start link.
    # Here is the requested logic: Link Visible ONLY if verified.
    # Since we can't force logic inside a channel post easily without a bot deep link, 
    # we will post the Direct Link but add a "Join First" text.
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "close_admin":
        ...
        await query.answer("📞 Contact: @YourAdminID", show_alert=True)
...
def main():
    ...
    print("🔥 FULL FEATURED BOT STARTED 🔥")
    app.run_polling()
